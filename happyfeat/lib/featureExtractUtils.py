...
def check_valid_replace_np(data):
    # Check if there are NaN values in the extracted data (power spectrum or anything else)
...
def check_nan_elec(data):
    # Detect channels with invalid values
    # Data must be of dimensions (trials x channels x frequencies)
    invalidElec = {}
    for trial in range(np.shape(data)[0]):
        # Check for electrodes that are fully NaN
        for elec in range(np.shape(data)[1]):
            if np.isnan(data[trial, elec, :]).any():
                if not invalidElec.get(elec):
                    invalidElec[elec] = 0
                invalidElec[elec] += 1
    return invalidElec

def check_invalid_trials_in_run(data):
    # Check if trials contain bad values, return list of
    # number of channels containing NaN (per trial)
    # Input data dimension is (trials x channels x frequencies)
    invalidTrials = []
    for trial in range(np.shape(data)[0]):
        # Check for electrodes that are fully NaN
        invalidCnt = 0
        for elec in range(np.shape(data)[1]):
            if np.isnan(data[trial, elec, :]).any():
                invalidCnt += 1
        invalidTrials.append(invalidCnt)
    return invalidTrials

def check_trial_for_invalid_elecs(trialData):
    # Check if input trial contain bad values, return
    # number of channels containing NaN values
    # Input data dimension is (channels x frequencies)
    invalidCnt = 0
    for elec in range(np.shape(trialData)[0]):
        if np.isnan(trialData[elec, :]).any():
            invalidCnt += 1
    return invalidCnt
